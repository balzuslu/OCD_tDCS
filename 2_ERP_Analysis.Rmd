...
```{r LMM-ERN-CRN, cache = knitr_cache_enabled, fig.width = 4, fig.height = 4, results = 'asis'}
...
plot_ERN_stim <- plot_model(LMM_ERN,
  ...
  scale_y_continuous(breaks=seq(-8, 2, 2), limits = c(-9,2), expand = c(0,0)) +
  scale_x_continuous(labels=c("HC","OCD"), breaks=c(1,2), limits=c(0.5,2.5), expand=c(0,0))
plot_ERN_stim
...
plot_CRN_stim <- plot_model(LMM_CRN,
  ...
  scale_y_continuous(breaks=seq(1, 9, 2), limits = c(1,9), expand = c(0,0))  +
  scale_x_continuous(labels=c("HC","OCD"), breaks=c(1,2), limits=c(0.5,2.5), expand=c(0,0))
plot_CRN_stim
...
# Plot predicted effects for ERN group and stimulation
plot_ERN_stim2 <- plot_model(LMM_ERN_P3_int_stim,
                                type   = "pred",
                                terms  = c("group", "stimulation"),
                                ci.lvl = .95) +
  labs(title = "Model-predicted ERN values",
...
# Save plot
ggsave("./figures/figure_LMM_ERN_control_P300_plot.tiff", width = 10, height = 10, units = "cm", dpi=300, compression = "lzw")
...
# Plot predicted effects for interaction P3 * group * stimulation
plot_P3_stimulation_group <- plot_model(LMM_ERN_P3,
                                type   = "pred",
                                terms  = c("P3_300_500_CPz_standardized","verum_sham","group"),
                                ci.lvl = .95) +
  labs(title = "ERN LMM: Plot interaction between P3 and group",
       x     = "P3 (standardized mean amplitude 300-500 ms at CPz [μV])",
       y     = "ERN (mean amplitude 0-100 ms at FCz [μV])") +
...
plot_Pe_stim <- plot_model(LMM_Pe,
  ...
  aes(color = group) +
  scale_color_manual(name = "Stimulation condition", labels = c("sham","cathodal"), values = c("slategray3", "navy")) +
  scale_y_continuous(breaks=seq(6, 14, 2), limits = c(6,14), expand = c(0,0)) + 
  scale_x_continuous(labels=c("HC","OCD"), breaks=c(1,2), limits=c(0.5,2.5), expand=c(0,0))
plot_Pe_stim
...
LMM_Pe_medication <- lmer(Pe_200_400_Pz ~ stimulation * group * medication   + 
  (1 + stimulation | participant_id),
data = single_trial_data_clean[single_trial_data_clean$response_type == "incorrect",],
REML = TRUE,
control = lmerControl(optimizer = "bobyqa")
...
plot_Pe_med_predicted1 <- plot_model(LMM_Pe_medication,
                                type   = "pred",
                                terms  = c("group","stimulation"),
                                ci.lvl = .95) +
  labs(title = "Predicted Pe amplitude",
       ...
       y     = "Pe [μV]") +
  my_figure_theme +
  scale_y_continuous(breaks=seq(6, 16, 2), limits = c(6,16), expand = c(0,0)) +
  scale_color_manual(name = "Stimulation condition", labels = c("sham","verum"), values = c("#ffad48", "#43b7c2"))
plot_Pe_med_predicted1


# Plot interaction stimulation * medication
plot_Pe_med_predicted2 <- plot_model(LMM_Pe_medication,
                                type   = "pred",
                                terms  = c("medication","stimulation"),
                                ci.lvl = .95) +
  labs(title = "Predicted Pe amplitude",
       x     = "Medication",
       y     = "Pe [μV]") +
  ...
  scale_color_manual(name = "Stimulation condition", labels = c("sham","verum"), values = c("#ffad48", "#43b7c2"))
plot_Pe_med_predicted2



# Pe check covariate medication (only additive)
LMM_Pe_medication_additive <- lmer(Pe_200_400_Pz ~ stimulation * group + medication   + 
  (1 + stimulation | participant_id),
data = single_trial_data_clean[single_trial_data_clean$response_type == "incorrect",],
REML = TRUE,
control = lmerControl(optimizer = "bobyqa")
...
# Convergence, singulatrity, PCA checked, all ok
tab_model(LMM_Pe_medication_additive,
  dv.labels = "Pe [μV], covariate medication (only additive)", show.stat = TRUE, show.icc = TRUE, show.r2 = TRUE, 
  show.re.var = FALSE, show.ngroups = TRUE, string.est = "b", string.stat = "t value", 
  string.ci = "95 % CI", string.p = "p value",  p.val = "satterthwaite", wrap.labels = 80, digits.re = 3
)
...
# Plot interaction stimulation * group 
plot_Pe_med_predicted1 <- plot_model(LMM_Pe_medication_additive,
                                type   = "pred",
                                terms  = c("group","stimulation"),
                                ci.lvl = .95) +
  labs(title = "Predicted Pe amplitude",
...
# Plot interaction stimulation * medication
plot_Pe_med_predicted2 <- plot_model(LMM_Pe_medication_additive,
                                type   = "pred",
                                terms  = c("medication","stimulation"),
                                ci.lvl = .95) +
  labs(title = "Predicted Pe amplitude",
