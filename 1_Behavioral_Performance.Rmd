...
performance_outliers <- df_aggregated_per_subject_accuracy %>% 
  ...
  mutate(outlier_2_sd_above = ifelse(abs(accuracy_numeric)>mean(accuracy_numeric)+2*sd(accuracy_numeric), TRUE, FALSE),
...
```
For some participants, accuracy in the task is more than 2 or even 3 SD below the group mean per condition (stimulus type x stimulation). But their accuracy is still quite good, so these will not be excluded. Being an outlier in task performance is also no exclusion criterion specified in the preregistration. 
<br><br>
