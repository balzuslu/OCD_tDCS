...
```{r topographies, fig.height = 10, fig.width = 10, dpi=600, fig.cap = "Note. Response-locked ERP topograhpies (from 0 to 100 ms) separately for both groups and response conditions."}
...
data_topo_correct  <- data_topo[data_topo$condition == "correct" ,]
data_topo_incorrect <- data_topo[data_topo$condition == "incorrect",]

data_topo_diff <- left_join(data_topo_correct, data_topo_incorrect, by = c("participant_id", "time", "group","session","stimulation","electrode")) %>%
...
plot_topo <- ggplot(data_topo, aes(x = x, y = y, fill = amplitude, label = electrode)) + 
    geom_topo(grid_res = 300, interp_limit = "head", chan_markers = "point", chan_size = 0.5, head_size = 0.9, groups = "group") + 
    ...
    theme(strip.text.x = element_text(margin = margin(0, 0, 0, 0))) + # to achive same text size with label for diff plot 
    theme(strip.text.y.left = element_text(angle = 0)) # and to show group labels horizontally
...
# Place labels separately in order to center them correctly
legend_topo      <- get_legend(plot_topo)
legend_topo_diff <- get_legend(plot_topo_diff)


# Remove previous legends from plots ( and some of the titles)
...
figure_ERP_topographies <- ggdraw() +
  draw_plot(plot_topo,        x =  0,  y =  0, width = .66, height = .95) +
  draw_plot(plot_topo_diff,   x = .66, y =  0, width = .33, height = .9)  +
  draw_plot(legend_topo,      x = .23, y =  0,  width = .2,  height = .05) + 
  draw_plot(legend_topo_diff, x = .7,  y =  0,  width = .2,  height = .05)


# Save plot
ggsave("./figures/figure_ERP_topographies.tiff", width = 20, height = 14, units = "cm", dpi=600, compression = "lzw")
